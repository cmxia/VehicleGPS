...
(function () {
    ...
    */
    MarkerClusterer.prototype._removeMarkersFromMap = function () {
        for (var i = 0, marker; marker = this._markers[i]; i++) {
            marker.isInCluster = false;
            var tmpLabel = marker.getLabel();//添加by miaoshaohao
            this._map.removeOverlay(marker);
            marker.setLabel(tmpLabel);//添加by miaoshaohao
        }
    };
    ...
    */
    Cluster.prototype.addMarker = function (marker) {
        ...
        if (len < this._minClusterSize) {
            ...
            for (var i = 0; i < len; i++) {
                var tmpLabel = this._markers[i].getLabel();//添加by miaoshaohao
                this._markers[i].getMap() && this._map.removeOverlay(this._markers[i]);
                this._markers[i].setLabel(tmpLabel);//添加by miaoshaohao
            }
    ...
    */
    Cluster.prototype.updateClusterMarker = function () {
        ...
        var markers = this._markers;//添加 by miaoshaohao
        var thatMap = this._map;
        var thatBounds = this.getBounds();
        this._clusterMarker.addEventListener("click", function (event) {
            //thatMap.setViewport(thatBounds);
            /*修改 by miaoshaohao*/
            if(event.overlay != null){ //判断覆盖物是否存在 
                return;
            }
            var len = markers.length;
            var ret = "";
            ret += "<table>"
            ret += "<tr><td style='font-size:14;color:blue;font-weight:bold'>";
            ret += len + "</td></tr>";
            ret += "<tr><td><table  style='font-size:12;color:black'>";
            for (var i = 0, m; m = markers[i]; i++) {
                if(i%9 == 0){
                    ret += "<tr>"
                }
                ret += "<td>"
                ret += markers[i].getTitle();
                ret += "</td>"
                if(i%9 == 8){
                    ret += "</tr>"
                }
            ...
            var t_InfoWindow = new BMap.InfoWindow(ret,{width:600}); // 创建信息窗口对象    
            thatMap.openInfoWindow(t_InfoWindow,new BMap.Point(event.target.getPosition().lng,event.target.getPosition().lat));      // 打开信息窗口 
        })
    };
