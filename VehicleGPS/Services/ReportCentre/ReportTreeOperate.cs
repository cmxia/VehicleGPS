...
namespace VehicleGPS.Services.ReportCentre
{
    class ReportTreeOperate
    {
        ...
        private void InitClientVehicleTree()
        {
            ...
            /*创建根节点*/
            ReportTreeNodeViewModel rootNode = new ReportTreeNodeViewModel();
            rootNode.NodeInfo = new CVBasicInfo();
            rootNode.NodeInfo.ID = "admin";
            rootNode.ListChildNodes = new List<ReportTreeNodeViewModel>();
            /*加载用户基础信息*/
            List<ReportTreeNodeViewModel> clientNodeList = new List<ReportTreeNodeViewModel>();
            ...
            {//等待10秒
                ...
                if (StaticTreeState.ClientBasicInfo == LoadingState.LOADCOMPLETE)
                {
                    ...
                    {
                        ReportTreeNodeViewModel clientNode = new ReportTreeNodeViewModel();
                    ...
                    {
                        foreach (ReportTreeNodeViewModel vtnv in clientNodeList)
                        {
            ...
            {//等待10秒超时或者加载用户信息失败
                ...
                ReportTreeViewModel.GetInstance().LoadTreeFail();
                return;
            }
            ReportTreeViewModel.GetInstance().RootNode = rootNode;//先显示用户树
            ...
            {//等待10秒
                ...
                {
                    /*获取与用户权限关联的车辆节点*/
                    List<ReportTreeNodeViewModel> vehicleNodeList = new List<ReportTreeNodeViewModel>();
                    foreach (CVBasicInfo vbi in basicInfo.ListVehicleBasicInfo)
                    {
                        ReportTreeNodeViewModel vehicleNode = new ReportTreeNodeViewModel();
                    ...
                    {
                        foreach (ReportTreeNodeViewModel vtnv in vehicleNodeList)
                        {
                            if (vtnv.isUsed == false)
                            {
            ...
            ReportTreeViewModel.GetInstance().RootNode = rootNode;
            ReportTreeViewModel.GetInstance().InitAutoComplete();/*初始化AutoComplete查询数据集*/
        ...
        #endregion
        /*获取未使用的节点*/
        private int UnUseCount(List<ReportTreeNodeViewModel> nodeList)
        {
            int count = 0;
            foreach (ReportTreeNodeViewModel vtnv in nodeList)
            {
                if (vtnv.isUsed == false)
                {
                    count++;
        ...
        public void InitCarOnlineState()
        {
            ...
            while (Monitor.TryEnter(StaticTreeState.VehicleGPSInfoMutex, 10000))
            {
                ...
                {
                    InitVehicleState(ReportTreeViewModel.GetInstance().rootNode);
                    Monitor.Exit(StaticTreeState.VehicleGPSInfoMutex);
                    break;
                }
            }
        ...
        /*计算用户下的总车辆数以及车辆状态*/
        private string[] InitVehicleState(ReportTreeNodeViewModel rootNode)
        {//返回类型string格式[1]count[2]onlineCount
            int count = 0;
            int onlineCount = 0;
            if (rootNode.ListChildNodes != null)
            {
                foreach (ReportTreeNodeViewModel vtnv in rootNode.ListChildNodes)
                {
        ...
        #region 用户节点
        /*添加单位、站点、车队节点*/
        private bool AddClientTree(ReportTreeNodeViewModel node, ReportTreeNodeViewModel t_node)
        {
            if (node.NodeInfo.ID == t_node.NodeInfo.ParentID)
            {
                if (node.ListChildNodes == null)
                {
                    node.ListChildNodes = new List<ReportTreeNodeViewModel>();
                }
            ...
            else
            {
                if (node.ListChildNodes != null)
                {
                    ...
                    {
                        ReportTreeNodeViewModel vtnv = node.ListChildNodes[i];
        ...
        /*初始化单位、车队、经营商、站点节点*/
        private void InitClientNode(ReportTreeNodeViewModel clientNode, CVBasicInfo cbi)
        {
        ...
        #region 车辆节点
        /*添加车辆节点*/
        private bool AddVehicleTree(ReportTreeNodeViewModel node, ReportTreeNodeViewModel t_node)
        {
            if (node.NodeInfo.ID == t_node.NodeInfo.ParentID)
            {
                if (node.ListChildNodes == null)
        ...
        /*初始化车辆节点*/
        private void InitVehicleNode(ReportTreeNodeViewModel vehicleNode, CVBasicInfo vbi)
        {
